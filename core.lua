...
-- Co-Author   : Tanku-Azuresong (Classic Era 1.15.8 compatibility and UI fixes)
local addonName, RIC = ...
...
function addon:OnInitialize()
	...
	local ok, frame = pcall(function()
		return LibStub("AceConfigDialog-3.0"):AddToBlizOptions(addonName) -- Add config menu to Blizzard options
	end)
	if ok then
		self.optionsFrame = frame
		RIC.optionsFrame = frame
	end

	RIC.OpenConfig = function()
		local aceDialog = LibStub("AceConfigDialog-3.0")
		if Settings and Settings.OpenToCategory and self.optionsFrame then
			local okOpen = pcall(function()
				if self.optionsFrame.ID then
					Settings.OpenToCategory(self.optionsFrame.ID)
				elseif self.optionsFrame.name then
		...
		aceDialog:Open(addonName)
		if aceDialog.SelectGroup then
			pcall(function() aceDialog:SelectGroup(addonName) end)
		end
	end
...
function addon:OnEnable() -- Called when the addon is enabled
	...
	local okGroup, errGroup = pcall(function() addon:OnEnableGroupview() end)
	if not okGroup then
		geterrorhandler()("RaidInviteClassic: failed to initialize group view: " .. tostring(errGroup))
	end
	local okRoster, errRoster = pcall(function() addon:OnEnableRosterManagerView() end)
	if not okRoster then
		geterrorhandler()("RaidInviteClassic: failed to initialize roster manager: " .. tostring(errRoster))
	end
	local okImport, errImport = pcall(function() addon:OnEnableImportView() end)
	if not okImport then
		geterrorhandler()("RaidInviteClassic: failed to initialize import view: " .. tostring(errImport))
	end
	local okMini, errMini = pcall(function() addon:OnEnableMinimap() end)
	if not okMini then
		geterrorhandler()("RaidInviteClassic: failed to initialize minimap: " .. tostring(errMini))
	end
