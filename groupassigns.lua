...
local function RIC_GetSafeGroupFont()
	local ok, fontObjFont = pcall(function()
		return GameFontHighlightSmall and GameFontHighlightSmall:GetFont()
	end)
	if ok and type(fontObjFont) == "string" and fontObjFont ~= "" then
		return fontObjFont
	end
	if type(STANDARD_TEXT_FONT) == "string" and STANDARD_TEXT_FONT ~= "" then
		return STANDARD_TEXT_FONT
	end
	if LSM and LSM.GetDefault and LSM.MediaTable and LSM.MediaTable.font then
		local key = LSM:GetDefault('font')
		local media = key and LSM.MediaTable.font[key]
		if type(media) == "string" and media ~= "" then
			return media
		end
	end
	return "Fonts\FRIZQT__.TTF"
end
local DEFAULT_FONT = RIC_GetSafeGroupFont()
...
function RIC:OnEnableGroupview()
	...
	self.groups:SetWidth(560)
	self.groups:SetHeight(620)
	...
	self.raidView:SetWidth(330)
	self.raidView:SetHeight(520)
	self.raidView:SetTitle("Raid overview")
	self.raidView:SetLayout("Flow")

	self.raidPlayerLabels = {}
	self.raidViewScroll = AceGUI:Create("ScrollFrame")
	self.raidViewScroll:SetLayout("Flow")
	self.raidView:AddChild(self.raidViewScroll)
	self.raidPlayerStatusLabels = {}
	self.raidGroups = {}

	for row = 1, 8 do
		local raidGroup = AceGUI:Create("InlineGroup")
		self.raidGroups[row] = raidGroup
		...
		AceGUI:RegisterLayout("RaidGroupLayout" .. row, function()
			for col = 1, 5 do
				...
				local rowStatusLabel = self.raidPlayerStatusLabels[row][col]
				if rowStatusLabel then
					rowStatusLabel:ClearAllPoints()
					rowStatusLabel:SetPoint("TOPLEFT", raidGroup.frame, "TOPLEFT", 140, dist + 1)
				end
			end
...
local function EnsureRaidStatusLabel(row, col)
	if not RIC.raidPlayerStatusLabels or not RIC.raidPlayerStatusLabels[row] then
		return nil
	end
	if RIC.raidPlayerStatusLabels[row][col] then
		return RIC.raidPlayerStatusLabels[row][col]
	end
	local raidGroup = RIC.raidGroups and RIC.raidGroups[row]
	...
	statusLabel:SetJustifyV("MIDDLE")
	raidGroup:AddChild(statusLabel)
	RIC.raidPlayerStatusLabels[row][col] = statusLabel
	return statusLabel
end
...
function RIC._Group_Manager.drawGroupLabels()
	...
	for row = 1, 8 do
		for col = 1, 5 do
			local label = RIC.raidPlayerLabels[row] and RIC.raidPlayerLabels[row][col]
			if label then
				local statusLabel = EnsureRaidStatusLabel(row, col)
				RIC._Group_Manager.SetLabel(label, statusLabel, filledPositions[(row-1)*5 + col])
			end
		end
	end
	if RIC.raidView and RIC.raidView.DoLayout then
		RIC.raidView:DoLayout()
	end
end
...
function RIC._Group_Manager.showPlayerBank()
	...
	while index < #RIC.playerBank.scroll.playerLabels do
		...
		RIC.playerBank.scroll.playerStatusLabels[index]:SetImage("Interface\AddOns\RaidInviteClassic\img\empty")
	end
